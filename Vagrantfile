...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sudo echo 192.168.56.200 ambari1.mycluster >> /etc/hosts
    sudo echo 192.168.56.201 master1.mycluster >> /etc/hosts
    sudo echo 192.168.56.202 slave1.mycluster >> /etc/hosts
    sudo echo 192.168.56.203 slave2.mycluster >> /etc/hosts
...
config.vm.define :ambari1 do |a1|
  ...
  a1.vm.network :private_network, ip: "192.168.56.200"
...
config.vm.define :master1 do |m1|
  ...
  m1.vm.network :private_network, ip: "192.168.56.201"
...
config.vm.define :slave1 do |s1|
  ...
  s1.vm.network :private_network, ip: "192.168.56.202"
...
config.vm.define :slave2 do |s2|
  ...
  s2.vm.network :private_network, ip: "192.168.56.203"
